...
try {
    ...
    $sql = "SELECT title as course_name, is_active FROM iit_jam_course_profiles WHERE is_active = 1 ORDER BY display_order ASC, title ASC";
    $stmt = $pdo->query($sql);
...
if (empty($heroSlides)) {
    $heroSlides = [
         [
            ...
            'subtitle' => 'Your trusted partner for Bank PO & SSC excellence.',
...
$pageContent = [];
try {
    $stmt = $pdo->query("SELECT section_key, title, content_value FROM iit_jam_content");
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $pageContent[$row['section_key']] = $row;
    }
} catch (Exception $e) {
    // Ignore
}

function getContent($key, $field = 'content_value') {
    global $pageContent;
    if (!isset($pageContent[$key][$field])) return '';
    $content = $pageContent[$key][$field];
    // If field is title, escape it. If content_value, allow HTML (Summernote).
    if ($field === 'title') {
        return htmlspecialchars($content);
    }
    return $content; // Return raw HTML for summernote content
}
...
<head>
    ...
    <!-- Site Title -->
    <title>Best Coaching for IIT JAM Mathematics | Infomaths</title>
...
<body>
            ...
            <div class="row align-items-center">
                ...
                <div class="col-lg-7 mb-5 mb-lg-0 wow fadeInUp" data-wow-delay="0.2s">
                    <div class="pro-breadcrumb">
                        ...
                        <span class="pro-breadcrumb-item active">IIT JAM Mathematics</span>
                    </div>

                    <h1 class="pro-banner-title"><?php echo getContent('hero_intro', 'title'); ?></h1>
                    <div class="pro-tagline">
                        <?php echo getContent('hero_intro', 'content_value'); ?>
                    </div>
                    ...
                    <div class="glass-form-card">
                        ...
                        <form action="#" method="POST" id="proHeroForm" class="unified-contact-form">
                            <div class="pro-input-group">
                                <input type="text" name="student_name" class="pro-input" placeholder="Your Name" required>
                            </div>

                            <div class="pro-input-group">
                                <input type="email" name="student_email" class="pro-input" placeholder="Email Address" required>
                            </div>
                            ...
                            <div class="pro-input-group d-flex gap-2">
                                ...
                                <input type="tel" name="student_mobile" class="pro-input" placeholder="Mobile Number" required>
                            </div>
                            ...
                            <div class="pro-input-group">
                                <textarea name="enquiry" class="pro-input" placeholder="Type your message" rows="3" style="resize: none;"></textarea>
                            </div>
                        ...
                        <div id="hero-form-message-pro" style="margin-top: 1rem; text-align: center; display: none;"></div>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <div class="td_card_info">
                            ...
                            <style>
                                ...
                                .tab-content-area {
                                    ...
                                    box-shadow: 0 5px 20px rgba(0,0,0,0.05); /* Added shadow for container */
                                }
                                ...
                                @media (max-width: 768px) {
                                    .custom-tabs {
                                        ...
                                        flex-direction: column;
                                        gap: 10px;
                                        background: transparent; /* Cleaner mobile look */
                                        padding: 0;
                                    }
                                    .custom-tabs .nav-link {
                                        width: 100%;
                                        ...
                                        margin-right: 0;
                                        background: #fff; /* White background for inactive mobile tabs */
                                    }
                                    .custom-tabs .nav-link.active {
                                        background-color: #1C56E1;
                                        color: white;
                            ...
                            $page_tabs = [];
                            try {
                                $stmt_tabs = $pdo->prepare("SELECT * FROM career_bca_page_tabs WHERE is_active = 1 ORDER BY display_order ASC");
                                $stmt_tabs->execute();
                                $page_tabs = $stmt_tabs->fetchAll(PDO::FETCH_ASSOC);
                            } catch (PDOException $e) {
                                // Error handling
                            }
                            ?>
                            ...
                            <div class="td_card_info_in">
                                <!-- Always Display Main Content -->
                                <h3 class="td_fs_32 td_mb_20"><?php echo getContent('main_content', 'title'); ?></h3>
                                <div class="td_mb_30 td_fs_18">
                                    <?php echo getContent('main_content', 'content_value'); ?>
                                </div>
                                <hr> <!-- Separator -->

                                <?php if (!empty($page_tabs)): ?>
                                    <!-- Dynamic Tabs -->
                                    <ul class="nav nav-pills custom-tabs mb-0" id="mainPageTabs" role="tablist">
                                        <?php foreach ($page_tabs as $key => $tab): ?>
                                    ...
                                    <div class="tab-content tab-content-area" id="mainPageTabsContent">
                                        ...
                                        <?php foreach ($page_tabs as $key => $tab): ?>
                                            <div class="tab-pane fade <?php echo $key === 0 ? 'show active' : ''; ?>" id="tab-content-<?php echo $tab['id']; ?>" role="tabpanel" aria-labelledby="tab-btn-<?php echo $tab['id']; ?>">
                                                <h3 class="td_fs_32 td_mb_20"><?php echo htmlspecialchars($tab['tab_title']); ?></h3>
                                                <div class="td_mb_30 td_fs_18">
                ...
                <div class="col-lg-4">
                    ...
                    <div class="td_card td_style_6 td_white_bg td_radius_10" style="padding: 40px 30px;">
                        ...
                        <h3 class="td_fs_20 td_semibold td_mb_10 mt-3">
                            University Board <span class="news-icon"><i class="fas fa-newspaper"></i></span>
                        </h3>
                        <div class="notice-box">
                            <ul class="notice-vertical">
                                ...
                                $notices = [];
                                try {
                                    $stmt = $pdo->query("SELECT * FROM iit_jam_notices WHERE is_active = 1 ORDER BY display_order ASC, created_at DESC");
                                ...
                                ?>
                                    <li><a href="#">No new updates available.</a></li>
                                <?php endif; ?>
                            </ul>
                        </div>
                    </div>

                    <!-- LATEST BANKING JOBS SECTION -->
                    <div class="td_card td_style_6 td_white_bg td_radius_10" style="padding: 40px 30px; margin-top: 30px;">

                        <h3 class="td_fs_20 td_semibold td_mb_10 mt-3">
                            Infomaths Notice <span class="news-icon"><i class="fas fa-briefcase"></i></span>
                        </h3>
                        <div class="notice-box">
                            <ul class="notice-vertical">
                                <?php
                                $jobs = [];
                                try {
                                    $stmt = $pdo->query("SELECT * FROM iit_jam_jobs WHERE is_active = 1 ORDER BY display_order ASC, created_at DESC");
                                ...
                                ?>
                                    <li>
                                        <a href="<?php echo htmlspecialchars($job['link']); ?>">
                                            <?php echo htmlspecialchars($job['content']); ?>
                                        </a>
                                    </li>
                                <?php 
                                    endforeach;
                    ...
                    <div class="td_card td_style_6 td_white_bg td_radius_10" style="padding: 40px 30px; margin-top:30px;">
                        ...
                        <div class="" style="display: flex; flex-wrap: wrap; gap:10px">
                            <div class="td_slider td_style_1" style="width: 100%;">
                                <div class="td_slider_container" style="width: 100%;" data-autoplay="1" data-loop="1" data-speed="800" data-center="0" data-variable-width="0" data-slides-per-view="1">
                    ...
                    <div class="megaleap-box-dark mb-4 mt-4" style="background: #1C56E1; border-radius: 12px; padding: 30px; color: #fff; position: relative; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,51,0.2);">
                        <!-- Decorative Circle -->
                        ...
                        <div style="position: relative; z-index: 2;">
                            ...
                            <p class="mb-4" style="color: rgba(255,255,255,0.9); font-size: 15px; line-height: 1.6;">
                                Join <strong>INFOMATHS' BANKING PRO</strong> — the premier course designed for Bank PO & SSC aspirants. 
                                <br><br>
                                <span style="display:block; border-left: 3px solid #fff; padding-left: 10px; font-style: italic; color: rgba(255,255,255,0.7);">
                                    Expert guidance, mock tests, and 24/7 personalized support for Banking/SSC Exams.
                                </span>
                            </p>
                            <a href="#" class="open-contact-modal" style="background: #fff; color: #1C56E1; padding: 10px 20px; border-radius: 6px; font-weight: 700; text-decoration: none; display: inline-block; transition: all 0.3s;" onmouseover="this.style.background='#f0f0f0'" onmouseout="this.style.background='#fff'">
    ...
    <section style="background-color: #f4f4f4;">
        <div class="container" style="padding: 80px 0;">
            <div class="td_section_heading td_style_1 text-center wow fadeInUp" data-wow-duration="1s" data-wow-delay="0.2s">
                ...
                <h2 class="td_section_title td_fs_48 mb-0 mt-4">Selected Candidates</h2>
            </div>
            ...
            <div class="td_slider td_style_1 td_slider_gap_24 wow fadeInUp" data-wow-duration="1s" data-wow-delay="0.3s">
                <div class="td_slider_container" data-autoplay="1" data-loop="1" data-speed="800" data-center="0" data-variable-width="0" data-slides-per-view="responsive" data-xs-slides="1" data-sm-slides="2" data-md-slides="3" data-lg-slides="4" data-add-slides="4">
                    <div class="td_slider_wrapper">
                        ...
                        try {
                            $stmt = $pdo->prepare("SELECT * FROM toppers WHERE category = 'IIT JAM' ORDER BY uploaded_at DESC");
                            $stmt->execute();
                            ...
                            if (!empty($nimcetToppers)) {
                                foreach ($nimcetToppers as $topper) {
                                    echo '<div class="td_slide">';
                                    echo '<div class="text-center" style="padding: 10px; background: #fff; border-radius: 10px; height: 100%;">'; // Added background and padding
                                    echo '<img src="assets/toppers/' . htmlspecialchars($topper['image_path']) . '" alt="' . htmlspecialchars($topper['name']) . '" style="width:100%; height:300px; object-fit:contain; border-radius:10px; margin:0 auto; display:block;">';
                                    echo '<h4 class="mt-3 mb-1" style="color: #1C56E1; font-weight: 700; font-size: 18px;">' . htmlspecialchars($topper['name']) . '</h4>';
                                    if (!empty($topper['description'])) {
                                        echo '<p style="color: #666; font-size: 14px; margin-bottom: 0;">' . htmlspecialchars($topper['description']) . '</p>';
                                    }
                                    echo '</div>';
                                    echo '</div>';
                                }
    ...
    <!-- MAHCET Toppers Section (Hidden for Bank PO) -->
    <!--
    ...
    <!-- VIT Toppers Section (Hidden for Bank PO) -->
    <!--
    <section>
        ...
    </section>
    ...
    <!-- PU Toppers Section (Hidden for Bank PO) -->
    <!--
    <section class="td_gray_bg_3">
        <div class="container" style="padding: 80px 0;">
            <div class="td_section_heading td_style_1 text-center wow fadeInUp" data-wow-duration="1s" data-wow-delay="0.2s">
            ...
            <div class="row d-flex align-items-stretch">
                <div class="col-12 col-md-6">
                    ...
                    <div class="td_slider td_style_1 h-100">
                        <div class="td_slider_container" data-autoplay="1" data-loop="1" data-speed="800" data-center="0" data-variable-width="0" data-slides-per-view="1" data-arrows="1" style="min-height: 500px;">
                            <div class="td_slider_wrapper">
                                <?php
                    ...
                    if (!empty($faculty_members)) {
                        foreach ($faculty_members as $member) {
                            ...
                            <div class="col-lg-3 col-md-4 col-sm-6 mb-4">
                                <div class="td_team_member text-center" style="background: #fff; padding: 25px; border-radius: 12px; box-shadow: 0 5px 20px rgba(0,0,0,0.05); transition: transform 0.3s ease;">
                                    <div class="td_member_img mb-3" style="position: relative; width: 120px; height: 120px; margin: 0 auto; overflow: hidden; border-radius: 50%; border: 3px solid #1C56E1;">
                                        <img src="assets/toppers/<?php echo htmlspecialchars($member['image_path']); ?>" alt="<?php echo htmlspecialchars($member['name']); ?>" style="width: 100%; height: 100%; object-fit: cover;">
                                    </div>
                                    <h4 class="td_member_name td_fs_20 td_mb_5" style="color: #1C56E1; font-weight: 700;"><?php echo htmlspecialchars($member['name']); ?></h4>
                                    <p class="td_member_designation td_fs_14 td_mb_0" style="color: #666;"><?php echo htmlspecialchars(substr($member['description'], 0, 50)); ?></p>
                                </div>
                            </div>
                            <?php
                        }
            ...
            <div class="container app-container_custom">
                ...
                <div class="td_cta_text_custom">
                    <div class="td_section_heading_custom">
                        ...
                        <p class="td_section_description_custom">Experience the future of entrance exam preparation with our comprehensive mobile app. Access study materials, practice tests, and expert guidance anytime, anywhere.</p>
                    </div>



                    <div class="td_btns_group_custom">
                        <a href="https://play.google.com/store/apps/details?id=co.jarvis.cedu&hl=en_IN" class="td_btn_custom">
    ...
    <section class="td_gray_bg_3">
        <div class="container" style="padding: 80px 0;">
            ...
            <div class="row td_gap_y_30">
                <?php
                try {
                    $stmt = $pdo->prepare("SELECT * FROM demo_lectures WHERE page_name = 'iit_jam' AND is_active = 1 ORDER BY display_order ASC LIMIT 4");
                    $stmt->execute();
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            async function generateOpenAISummary(query, searchContent) {
                ...
                const prompt = `You are an expert education assistant. Summarize the following search results for the query: "${query}".\n\n` +
        ...
        $(document).ready(function() {
            ...
            if (heroContactForm) {
                heroContactForm.addEventListener('submit', function(e) {
                    ...
                    // Basic validation
                    if (!data.student_name || !data.student_email || !data.student_mobile || !data.student_state || !data.student_city || !data.course_interest) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (proHeroForm) {
                proHeroForm.addEventListener('submit', function(e) {
                    ...
                    // Basic validation
                    if (!data.student_name || !data.student_email || !data.student_mobile || !data.course_interest) {
                    ...
                    submitBtn.innerHTML = '<span>Sending...</span> <div class="spinner-border spinner-border-sm ms-2" role="status"></div>';
                    submitBtn.disabled = true;
