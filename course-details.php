...
try {
    if (!empty($slug)) {
        $stmt = $pdo->prepare("SELECT * FROM course_profiles WHERE slug = ?");
        $stmt->execute([$slug]);
        $course = $stmt->fetch(PDO::FETCH_ASSOC);
    } elseif ($id > 0) {
        $stmt = $pdo->prepare("SELECT * FROM course_profiles WHERE id = ?");
        $stmt->execute([$id]);
        $course = $stmt->fetch(PDO::FETCH_ASSOC);
    }
...
<head>
    ...
    <meta name="keywords" content="<?php echo !empty($course['meta_keyword']) ? htmlspecialchars($course['meta_keyword']) : 'mca entrance, infomaths, ' . htmlspecialchars($course['title']); ?>">
