...
try {
    if (!empty($slug)) {
        // CHANGED: Use bank_po_entrance_exams table
        $stmt = $pdo->prepare("SELECT * FROM bank_po_entrance_exams WHERE slug = ?");
        $stmt->execute([$slug]);
        $exam = $stmt->fetch(PDO::FETCH_ASSOC);
    } elseif ($id > 0) {
        // CHANGED: Use bank_po_entrance_exams table
        $stmt = $pdo->prepare("SELECT * FROM bank_po_entrance_exams WHERE id = ?");
        $stmt->execute([$id]);
        $exam = $stmt->fetch(PDO::FETCH_ASSOC);
    }
...
<head>
    ...
    <meta name="keywords" content="<?php echo !empty($exam['meta_keyword']) ? htmlspecialchars($exam['meta_keyword']) : 'bank po, ssc, infomaths, ' . htmlspecialchars($exam['exam_name']); ?>">
    ...
    <style>
        ...
        /* Professional Tabs Styling - Refined */
        .custom-tabs {
            border-bottom: 2px solid #1C56E1;
        }
        ...
        .custom-tabs .nav-link.active {
            background-color: #1C56E1; /* Brand Blue */
            color: white;
            ...
            <div class="row justify-content-center">
                <div class="col-lg-10">
                    <div class="course-card-shadow animate__animated animate__fadeInUp" style="animation-delay: 0.3s;">
                        ...
                        $tabs = [];
                        try {
                            $stmt_tabs = $pdo->prepare("SELECT * FROM bank_po_exam_tabs WHERE exam_id = ? ORDER BY display_order ASC");
