...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_profile'])) {
    ...
    if (empty($error_message)) {
        if (empty($title) || empty($subtitle)) {
            ...
            try {
                $stmt = $pdo->prepare("INSERT INTO bank_po_course_profiles (title, subtitle, link, description, slug, meta_title, meta_keyword, meta_description, display_order, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit_profile'])) {
    ...
    if ($link_type === 'external_url') {
        ...
        // If no new file, keep existing link (which is already in $link from hidden input if we passed it, or we need to query it? 
        // Actually, the JS populates #edit_link with the *current* DB value. 
        // But the user might have changed the radio to PDF from CMS. In that case existing link is '#' which is wrong for PDF.
        // If switching TO PDF without upload, we can't really set a valid PDF link unless we keep old one. 
        // Logic: If user stays on PDF and doesn't upload, keep old. If switches to PDF, they MUST upload.
        
        // However, simplify: If no new file uploaded, use existing only if it looks like a PDF? 
        // Or just rely on what is submitted.
        
        // Refinement: The hidden 'edit_link' input holds the OLD value.
        // If user changed type to PDF, they should upload a file. 
        // If they didn't upload a file, and old link was NOT pdf, we have an issue.
    ...
    if (empty($error_message)) {
        if (empty($title) || empty($subtitle)) {
            ...
            try {
                $stmt = $pdo->prepare("UPDATE bank_po_course_profiles SET title = ?, subtitle = ?, link = ?, description = ?, slug = ?, meta_title = ?, meta_keyword = ?, meta_description = ?, display_order = ?, is_active = ? WHERE id = ?");
...
if (isset($_GET['delete_id'])) {
    ...
    try {
        $stmt = $pdo->prepare("DELETE FROM bank_po_course_profiles WHERE id = ?");
        $stmt->execute([$id]);
...
// Fetch all profiles
try {
    $stmt = $pdo->query("SELECT * FROM bank_po_course_profiles ORDER BY display_order ASC");
...
<head>
    ...
    <meta name="description" content="Info Maths Online admin dashboard - Manage Syllabus.">
    
    <!-- FAVICON -->
    <link rel="shortcut icon" type="image/x-icon" href="../assets/img/favicon.png">

    <!-- TITLE -->
    <title>Info Maths Online - Syllabus Management</title>
...
<body class="app sidebar-mini ltr light-mode">
                        ...
                        <div class="page-header">
                            ...
                            <div>
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item"><a href="javascript:void(0);">Bank PO & SSC Page</a></li>
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-bordered text-nowrap border-bottom" id="basic-datatable">
                                                ...
                                                <tbody>
                                                    ...
                                                    <?php else: ?>
                                                        <tr>
                                                            <td colspan="6" class="text-center">No syllabus found.</td>
                                                        </tr>
                                                    <?php endif; ?>
                                                </tbody>
                                            </table>
    ...
    <script>
        $(document).ready(function() {
            ...
            // --- ADD MODAL LOGIC ---
            // Handle Link Type Change in Add Modal
            ...
            // --- EDIT MODAL LOGIC ---
            // Handle Link Type Change in Edit Modal
            ...
            $('.edit-profile-btn').on('click', function() {
                ...
                // Determine Link Type based on existing data
                // Heuristic:
                // If link is '#' or empty(but slug exists), it's likely CMS Page.
                // If link ends in .pdf, it's PDF.
                // Else it's External/Internal.
                ...
                if ((link === '#' || link === '') && slug) {
                    ...
                    linkType = 'pdf';
                    // Show current PDF link if needed? 
            ...
            function handleLinkTypeChange(type, mode) {
                ...
                if (type === 'cms_page') {
                    ...
                    // In Add mode, clear external link if any
                    if(mode === 'add') $(prefix + 'external_link').val('#'); 
                } 
                else if (type === 'external_url') {
                    $(prefix + 'container_link_external').show();
                    // In external mode, slug/desc are not primary but we might keep them empty
                    // We DO NOT show editor or slug usually for external links in this context to keep it simple, 
                    // or we might optionaly show them. Let's hide to simplify as requested "Give options like... target=_blank"
                } 
                else if (type === 'pdf') {
                    $(prefix + 'container_pdf').show();
                    // PDF also doesn't need slug/desc usually, just title/subtitle on card -> pdf
                }
            }
            
            // Helper for internal links (Select2) could be added here if we had a list of pages
            // For now 'external_url' field can accept relative paths too.
        });
    </script>
</body>
</html>
