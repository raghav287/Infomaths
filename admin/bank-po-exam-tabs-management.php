<?php
// admin/bank-po-exam-tabs-management.php
session_start();
...
if (!isset($_GET['exam_id'])) {
    header("location: bank-po-exam-management.php");
    exit;
}
...
// Fetch Exam Name
try {
    $stmt = $pdo->prepare("SELECT exam_name FROM bank_po_entrance_exams WHERE id = ?");
    $stmt->execute([$exam_id]);
    $exam = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($exam) {
        $exam_name = $exam['exam_name'];
    } else {
        header("location: bank-po-exam-management.php"); // Exam not found
        exit;
    }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_tab'])) {
    ...
    if (empty($title)) {
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO bank_po_exam_tabs (exam_id, tab_title, tab_content, display_order) VALUES (?, ?, ?, ?)");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit_tab'])) {
    ...
    if (empty($title)) {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE bank_po_exam_tabs SET tab_title = ?, tab_content = ?, display_order = ? WHERE id = ? AND exam_id = ?");
...
if (isset($_GET['delete_id'])) {
    ...
    try {
        $stmt = $pdo->prepare("DELETE FROM bank_po_exam_tabs WHERE id = ? AND exam_id = ?");
...
// Fetch Tabs
try {
    $stmt = $pdo->prepare("SELECT * FROM bank_po_exam_tabs WHERE exam_id = ? ORDER BY display_order ASC");
    $stmt->execute([$exam_id]);
                ...
                <div class="side-app">
                    <div class="main-container container-fluid">
                        <div class="page-header">
                            ...
                            <div>
                                <a href="bank-po-exam-management.php" class="btn btn-secondary"><i class="fa fa-arrow-left"></i> Back to Exams</a>
                            </div>
                        </div>
