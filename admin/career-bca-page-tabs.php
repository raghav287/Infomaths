<?php
// admin/career-bca-page-tabs.php
session_start();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_tab'])) {
    ...
    if (empty($title)) {
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO bca_page_tabs (tab_title, tab_content, display_order) VALUES (?, ?, ?)");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit_tab'])) {
    ...
    if (empty($title)) {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE bca_page_tabs SET tab_title = ?, tab_content = ?, display_order = ? WHERE id = ?");
...
if (isset($_GET['delete_id'])) {
    ...
    try {
        $stmt = $pdo->prepare("DELETE FROM bca_page_tabs WHERE id = ?");
        $stmt->execute([$id]);
...
// Fetch Tabs
try {
    $stmt = $pdo->prepare("SELECT * FROM bca_page_tabs ORDER BY display_order ASC");
    $stmt->execute();
...
<head>
    ...
    <title>Manage Career BCA Page Tabs</title>
                ...
                <div class="side-app">
                    <div class="main-container container-fluid">
                        <div class="page-header">
                            <h1 class="page-title">Manage Career BCA <span class="text-primary">Page Tabs</span></h1>
                            <div>
                                <ol class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="panel">Home</a></li>
                                    <li class="breadcrumb-item active" aria-current="page">Career BCA Tabs</li>
                                </ol>
                            </div>
                        </div>
