...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_tab'])) {
    ...
    if (empty($title)) {
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO csir_net_page_tabs (tab_title, tab_content, display_order) VALUES (?, ?, ?)");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit_tab'])) {
    ...
    if (empty($title)) {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE csir_net_page_tabs SET tab_title = ?, tab_content = ?, display_order = ? WHERE id = ?");
...
if (isset($_GET['delete_id'])) {
    ...
    try {
        $stmt = $pdo->prepare("DELETE FROM csir_net_page_tabs WHERE id = ?");
        $stmt->execute([$id]);
...
// Fetch Tabs
try {
    $stmt = $pdo->prepare("SELECT * FROM csir_net_page_tabs ORDER BY display_order ASC");
    $stmt->execute();
...
<head>
    ...
    <title>Manage CSIR NET Page Tabs</title>
                ...
                <div class="side-app">
                    <div class="main-container container-fluid">
                        <div class="page-header">
                            <h1 class="page-title">Manage CSIR NET <span class="text-primary">Page Tabs</span></h1>
                            <div>
                                <ol class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="panel">Home</a></li>
                                    <li class="breadcrumb-item active" aria-current="page">CSIR NET Tabs</li>
                                </ol>
                            </div>
                        </div>
