...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_profile'])) {
    ...
    if (empty($error_message)) {
        if (empty($title) || empty($subtitle)) {
            ...
            try {
                $stmt = $pdo->prepare("INSERT INTO csir_net_course_profiles (title, subtitle, link, description, slug, meta_title, meta_keyword, meta_description, display_order, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit_profile'])) {
    ...
    if (empty($error_message)) {
        if (empty($title) || empty($subtitle)) {
            ...
            try {
                $stmt = $pdo->prepare("UPDATE csir_net_course_profiles SET title = ?, subtitle = ?, link = ?, description = ?, slug = ?, meta_title = ?, meta_keyword = ?, meta_description = ?, display_order = ?, is_active = ? WHERE id = ?");
...
if (isset($_GET['delete_id'])) {
    ...
    try {
        $stmt = $pdo->prepare("DELETE FROM csir_net_course_profiles WHERE id = ?");
        $stmt->execute([$id]);
...
// Fetch all profiles
try {
    $stmt = $pdo->query("SELECT * FROM csir_net_course_profiles ORDER BY display_order ASC");
...
<head>
    ...
    <meta name="description" content="Info Maths Online admin dashboard - Manage CSIR NET Syllabus.">
    
    <!-- FAVICON -->
    <link rel="shortcut icon" type="image/x-icon" href="../assets/img/favicon.png">

    <!-- TITLE -->
    <title>Info Maths Online - CSIR NET Syllabus Management</title>
                ...
                <div class="side-app">
                    ...
                    <div class="main-container container-fluid">
                        ...
                        <div class="page-header">
                            <h1 class="page-title">CSIR NET Syllabus Management</h1>
                            <div>
                                <ol class="breadcrumb">
                            ...
                            <div class="col-lg-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h3 class="card-title">Manage CSIR NET Syllabus</h3>
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-bordered text-nowrap border-bottom" id="basic-datatable">
                                                ...
                                                <tbody>
                                                    ...
                                                    <?php else: ?>
                                                        <tr>
                                                            <td colspan="6" class="text-center">No syllabus found for CSIR NET.</td>
                                                        </tr>
                                                    <?php endif; ?>
                                                </tbody>
                                            </table>
