...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_exam'])) {
    ...
    if (empty($error_message)) {
        if (empty($exam_name)) {
            ...
            try {
                $stmt = $pdo->prepare("INSERT INTO bank_po_entrance_exams (exam_name, slug, icon_image, short_description, full_description, meta_title, meta_keyword, meta_description, display_order, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit_exam'])) {
    ...
    if (!empty($_FILES['edit_icon_image']['name'])) {
        ...
        if ($upload_result['success']) {
            ...
            try {
                $stmt = $pdo->prepare("UPDATE bank_po_entrance_exams SET exam_name = ?, slug = ?, icon_image = ?, short_description = ?, full_description = ?, meta_title = ?, meta_keyword = ?, meta_description = ?, display_order = ?, is_active = ? WHERE id = ?");
        ...
        try {
            $stmt = $pdo->prepare("UPDATE bank_po_entrance_exams SET exam_name = ?, slug = ?, short_description = ?, full_description = ?, meta_title = ?, meta_keyword = ?, meta_description = ?, display_order = ?, is_active = ? WHERE id = ?");
...
if (isset($_GET['delete_id'])) {
    ...
    try {
        $stmt = $pdo->prepare("DELETE FROM bank_po_entrance_exams WHERE id = ?");
        $stmt->execute([$id]);
...
// Fetch all exams
try {
    $stmt = $pdo->query("SELECT * FROM bank_po_entrance_exams ORDER BY display_order ASC");
...
<head>
    ...
    <!-- TITLE -->
    <title>Info Maths Online - Bank PO Exam Management</title>
                ...
                <div class="side-app">
                    ...
                    <div class="main-container container-fluid">
                        ...
                        <div class="page-header">
                            <h1 class="page-title">Bank PO Exam Management</h1>
                            <div>
                                <ol class="breadcrumb">
                            ...
                            <div class="col-lg-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h3 class="card-title">Manage Bank PO Exams</h3>
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <div class="btn-group" role="group">
                                                                        <a href="bank-po-exam-tabs-management.php?exam_id=<?php echo $exam['id']; ?>" class="btn btn-info btn-sm">
