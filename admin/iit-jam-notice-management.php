...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['add_notice'])) {
        ...
        if ($link_type == 'page') {
            ...
            $counter = 1;
            while(true) {
                $check = $pdo->prepare("SELECT id FROM iit_jam_notices WHERE slug = ?");
                $check->execute([$slug]);
        ...
        if (!empty($content)) {
            try {
                $stmt = $pdo->prepare("INSERT INTO iit_jam_notices (content, link, display_order, is_active, slug, page_title, page_content, meta_title, meta_description) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    if (isset($_POST['edit_notice'])) {
        ...
        // Fetch existing data
        $existing = $pdo->prepare("SELECT * FROM iit_jam_notices WHERE id = ?");
        $existing->execute([$id]);
        ...
        if ($link_type == 'page') {
             ...
             $counter = 1;
             while(true) {
                 $check = $pdo->prepare("SELECT id FROM iit_jam_notices WHERE slug = ? AND id != ?");
        ...
        if (!empty($content)) {
            try {
                $stmt = $pdo->prepare("UPDATE iit_jam_notices SET content = ?, link = ?, display_order = ?, is_active = ?, slug = ?, page_title = ?, page_content = ?, meta_title = ?, meta_description = ? WHERE id = ?");
    ...
    if (isset($_POST['delete_id'])) {
        ...
        try {
            $stmt = $pdo->prepare("DELETE FROM iit_jam_notices WHERE id = ?");
            $stmt->execute([$id]);
...
// Fetch Notices
$notices = $pdo->query("SELECT * FROM iit_jam_notices ORDER BY display_order ASC, created_at DESC")->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    ...
    <title>Info Maths Online - IIT JAM Notice Management</title>
                ...
                <div class="side-app">
                    <div class="main-container container-fluid">
                        <div class="page-header">
                            <h1 class="page-title">IIT JAM Notice Management</h1>
                            <div>
                                <ol class="breadcrumb">
                            ...
                            <div class="col-lg-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h3 class="card-title">Manage IIT JAM Notices</h3>
