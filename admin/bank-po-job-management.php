...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['add_job'])) {
        ...
        if ($link_type == 'page') {
            ...
            $counter = 1;
            while(true) {
                $check = $pdo->prepare("SELECT id FROM bank_po_jobs WHERE slug = ?");
                $check->execute([$slug]);
        ...
        if (!empty($content)) {
            try {
                $stmt = $pdo->prepare("INSERT INTO bank_po_jobs (content, link, display_order, is_active, slug, page_title, page_content, meta_title, meta_description) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    if (isset($_POST['edit_job'])) {
        ...
        // Fetch existing data
        $existing = $pdo->prepare("SELECT * FROM bank_po_jobs WHERE id = ?");
        $existing->execute([$id]);
        ...
        if ($link_type == 'page') {
             ...
             $counter = 1;
             while(true) {
                 $check = $pdo->prepare("SELECT id FROM bank_po_jobs WHERE slug = ? AND id != ?");
        ...
        if (!empty($content)) {
            try {
                $stmt = $pdo->prepare("UPDATE bank_po_jobs SET content = ?, link = ?, display_order = ?, is_active = ?, slug = ?, page_title = ?, page_content = ?, meta_title = ?, meta_description = ? WHERE id = ?");
    ...
    if (isset($_POST['delete_id'])) {
        ...
        try {
            $stmt = $pdo->prepare("DELETE FROM bank_po_jobs WHERE id = ?");
            $stmt->execute([$id]);
...
// Fetch Jobs
$jobs = $pdo->query("SELECT * FROM bank_po_jobs ORDER BY display_order ASC, created_at DESC")->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    ...
    <title>Bank PO Job Management - Admin</title>
...
<body class="app sidebar-mini ltr light-mode">
    ...
    <div class="page">
                ...
                <div class="side-app">
                    <div class="main-container container-fluid">
                        <div class="page-header">
                            <h1 class="page-title">Bank PO Job Management</h1>
                            <div>
                                <ol class="breadcrumb">
                                            ...
                                            <table class="table table-bordered text-nowrap border-bottom" id="basic-datatable">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th class="wd-40p">Job Title / Content</th>
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        <td><?php echo $job['display_order']; ?></td>
                                                        <td style="white-space:normal;"><?php echo htmlspecialchars($job['content']); ?></td>
                                                        <td>
                                                            <?php if(!empty($job['slug'])): ?>
                                                                <span class="badge bg-purple">CMS Page</span><br>
                                                                <small><a href="../job/<?php echo htmlspecialchars($job['slug']); ?>" target="_blank" class="text-primary">View Page</a></small>
                                                            <?php else: ?>
                                                                <span class="badge bg-secondary">Ext Link</span><br>
                                                                <small><a href="../<?php echo htmlspecialchars($job['link']); ?>" target="_blank" class="text-muted"><?php echo substr($job['link'],0,20); ?>...</a></small>
                                                            <?php endif; ?>
                                                        </td>
                                                        <td>
                                                        ...
                                                        <td>
                                                            <button class="btn btn-sm btn-warning edit-btn" 
                                                                    ...
                                                                    data-content="<?php echo htmlspecialchars($job['content']); ?>"
                                                                    data-link="<?php echo htmlspecialchars($job['link']); ?>"
                                                                    data-order="<?php echo $job['display_order']; ?>"
                                                                    data-active="<?php echo $job['is_active']; ?>"
                                                                    data-pagetitle="<?php echo htmlspecialchars($job['page_title']); ?>"
                                                                    data-metatitle="<?php echo htmlspecialchars($job['meta_title']); ?>"
                                                                    data-metadesc="<?php echo htmlspecialchars($job['meta_description']); ?>"
                                                                    data-slug="<?php echo htmlspecialchars($job['slug']); ?>">
                                                                <i class="fe fe-edit"></i> Edit
                                                            </button>
                                                            <!-- Hidden textarea to store HTML content safely -->
                                                            <textarea id="content_html_<?php echo $job['id']; ?>" style="display:none;"><?php echo htmlspecialchars($job['page_content']); ?></textarea>
                ...
                <div class="modal-content">
                    <form method="POST">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                <label class="form-label">Job Title / Text <span class="text-danger">*</span></label>
                                <textarea name="content" class="form-control" rows="2" required placeholder="Enter job title or notice text..."></textarea>
                            </div>
        ...
        <!-- Edit Modal -->
        <div class="modal fade" id="editJobModal" tabindex="-1" data-bs-backdrop="static">
                        ...
                        <div class="modal-body">
                            ...
                            <div id="edit_page_section" style="display:none; border: 1px solid #eee; padding: 15px; border-radius: 5px; background: #fafafa;">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="slug" id="edit_slug" class="form-control" placeholder="e.g. my-job-title">
