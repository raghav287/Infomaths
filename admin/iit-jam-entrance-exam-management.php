...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_exam'])) {
    ...
    if (empty($error_message)) {
        if (empty($exam_name)) {
            ...
            try {
                $stmt = $pdo->prepare("INSERT INTO iit_jam_entrance_exams (exam_name, slug, icon_image, short_description, full_description, meta_title, meta_keyword, meta_description, display_order, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit_exam'])) {
    ...
    if (!empty($_FILES['edit_icon_image']['name'])) {
        ...
        if ($upload_result['success']) {
            ...
            try {
                $stmt = $pdo->prepare("UPDATE iit_jam_entrance_exams SET exam_name = ?, slug = ?, icon_image = ?, short_description = ?, full_description = ?, meta_title = ?, meta_keyword = ?, meta_description = ?, display_order = ?, is_active = ? WHERE id = ?");
        ...
        try {
            $stmt = $pdo->prepare("UPDATE iit_jam_entrance_exams SET exam_name = ?, slug = ?, short_description = ?, full_description = ?, meta_title = ?, meta_keyword = ?, meta_description = ?, display_order = ?, is_active = ? WHERE id = ?");
...
if (isset($_GET['delete_id'])) {
    ...
    try {
        $stmt = $pdo->prepare("DELETE FROM iit_jam_entrance_exams WHERE id = ?");
        $stmt->execute([$id]);
...
// Fetch all exams
try {
    $stmt = $pdo->query("SELECT * FROM iit_jam_entrance_exams ORDER BY display_order ASC");
...
<head>
    ...
    <meta name="description" content="Info Maths Online admin dashboard - Manage IIT JAM Entrance Exams.">
    
    <!-- FAVICON -->
    <link rel="shortcut icon" type="image/x-icon" href="../assets/img/favicon.png">

    <!-- TITLE -->
    <title>Manage IIT JAM Exams</title>
                ...
                <div class="side-app">
                    ...
                    <div class="main-container container-fluid">
                        ...
                        <div class="page-header">
                            <h1 class="page-title">IIT JAM Entrance Exams</h1>
                            <div>
                                <ol class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="panel">Home</a></li>
                                    <li class="breadcrumb-item">IIT JAM Page</li>
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <div class="btn-group" role="group">
                                                                        <a href="iit-jam-exam-tabs-management.php?exam_id=<?php echo $exam['id']; ?>" class="btn btn-info btn-sm">
                                                                            <i class="fe fe-layers"></i> Manage Tabs
                                                                        </a>
                        ...
                        <div class="modal-body">
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control" id="exam_name" name="exam_name" required placeholder="e.g. TIFR">
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label for="slug" class="form-label">Slug (URL)</label>
                                    <input type="text" class="form-control" id="slug" name="slug" placeholder="e.g. tifr-exam">
                                </div>
                            </div>
                            ...
                            <div class="mb-3">
                                <label for="full_description" class="form-label">Full Details</label>
